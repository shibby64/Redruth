        ...
        .then((object) => {
            if (object.success && object.results) {
                ...
                for (i = 0; i < object.results.length; i++) {
                    if (object.results[i].public_flg) {
                        publicStories[j] = object.results[i];
                        j++;
                    }
                }
...
function metaData(publicStories) {
    ...
    for (let m = 0; m < publicStories.length; m++) {
        ...
        const commentNode = document.createTextNode("* Comments: " + publicStories[m].remarks);
        comment.appendChild(commentNode);
        recordingElement.append(comment);
        /*-----------------------------------------------------------*/
        const timeStamp = document.createElement('p');
        timeStamp.classList.add('metaDataStyle');
        const timeStampNode = document.createTextNode("* Timestamp: " + publicStories[m].timestamp);
        timeStamp.appendChild(timeStampNode);
        recordingElement.append(timeStamp);
        /*-----------------------------------------------------------*/
        if (publicStories[m].postal_code !== null && publicStories[m].postal_code !== "") {
            const postCode = document.createElement('p');
            postCode.classList.add('metaDataStyle');
            const postCodeNode = document.createTextNode("* Post Code: " + publicStories[m].postal_code);
        ...
        audioTag.innerHTML = '<audio id="audio-player" controls="controls" src= ' + publicStories[m].filepath + ' type="audio/mpeg">';
